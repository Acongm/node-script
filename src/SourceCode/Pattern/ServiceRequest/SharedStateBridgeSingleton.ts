...
export const unsubscribeSharedData = singleton.unsubscribeSharedData.bind(singleton);

// ========== 工单组件缓存 ==========
export const getCachedServiceRequest = componentRegistry.get.bind(componentRegistry);
export const setCachedServiceRequest = componentRegistry.set.bind(componentRegistry);
...
export const registerFormDataCallback = formCallbackRegistry.set.bind(formCallbackRegistry);
export const getFormDataCallback = formCallbackRegistry.get.bind(formCallbackRegistry);
export const unregisterFormDataCallback = formCallbackRegistry.delete.bind(formCallbackRegistry);

// ========== 资源更新回调（单回调） ==========
const resourceCallbackRegistry = new Registry<ResourceUpdateCallback>();
const RESOURCE_KEY = 'update';
export const registerResourceUpdateCallback = (callback: ResourceUpdateCallback) =>
...
export const triggerResourceAdd = (resourceType: string) => {
  ...
  if (callback) {
    ...
    console.warn(`No resource update callback registered, cannot add ${resourceType}`);
  }
};