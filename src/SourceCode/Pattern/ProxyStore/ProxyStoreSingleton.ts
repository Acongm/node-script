...
const legacyWrapperMap = new Map<string, WeakMap<Function, ProxyStoreSubscriber>>();
 ...
 */
export function watch<T>(source: () => T, cb: (value: T, oldValue: T | undefined) => void, options: WatchOptions = {}): () => void {
...
export const subscribe = (key: string, subscriber: ProxyStoreSubscriber): (() => void) => {
  return store.subscribe(key, subscriber);
};

export const unsubscribe = (key: string, subscriber: ProxyStoreSubscriber): void => {
