...
export const on = <T>(event: string, handler: EventHandler<T>, immediate = true): void => {
 ...
 * 订阅共享数据变化
 */
export const subscribeSharedData = <T>(key: string, callback: SharedDataSubscriber<T>): void => {
  on(key, callback);
};
 ...
 * 取消订阅共享数据变化
 */
export const unsubscribeSharedData = <T>(key: string, callback: SharedDataSubscriber<T>): void => {
  off(key, callback);
};
 ...
 * 一次性订阅
 */
export const onceSharedData = <T>(key: string, callback: SharedDataSubscriber<T>): void => {
  once(key, callback);
};
