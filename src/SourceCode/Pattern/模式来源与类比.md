...
```ts
import { setState, getState, subscribe, batchUpdate, getSnapshot } from './Store/StoreSingleton';
...
```ts
import { publish, subscribe, clear } from './BroadcastChannel/BroadcastSingleton';
...
const stop = watch(() => state.count, (value) => console.log('count:', value), { immediate: true });
