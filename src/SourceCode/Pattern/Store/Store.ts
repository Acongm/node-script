...
export type StoreSubscriber<T = any> = (data: T, prevData: T | undefined) => void;
...
export class Store<T extends Record<string, any> = Record<string, any>> {
   ...
   * 订阅状态变化
   */
  subscribe<K extends keyof T>(key: K, subscriber: StoreSubscriber<T[K]>): () => void {
   ...
   * 取消订阅
   */
  unsubscribe<K extends keyof T>(key: K, subscriber: StoreSubscriber<T[K]>): void {
   ...
   * 通知订阅者
   */
  private notifySubscribers<K extends keyof T>(key: K, value: T[K], prevValue: T[K] | undefined): void {
    this.subscribers.get(key)?.forEach(subscriber => subscriber(value, prevValue));
  }
