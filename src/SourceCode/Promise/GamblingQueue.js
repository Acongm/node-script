...
class GamblingQueue {
  ...
  playRound () {
    ...
    const roundData = {
      ...
      betBig: isBetBig,
      result: null,
      outcomeInfo: null // 用于存储详细的开奖原因 (运气/出千/冲突)
    };
    ...
    setTimeout(() => {
      ...
      let finalOutcome = 'normal'; // 'normal', 'cheat', 'lucky', 'conflict_win', 'conflict_loss'

      // 冲突解决：当庄家出千遇上运气爆棚
      if (isCheating && isLucky) {
        if (this.luckProbability > this.cheatProbability) {
          finalOutcome = 'conflict_win';
        } else {
          finalOutcome = 'conflict_loss';
        }
      } else if (isCheating) {
        finalOutcome = 'cheat';
      } else if (isLucky) {
      ...
      if (finalOutcome === 'cheat' || finalOutcome === 'conflict_loss') {
        // 出老千/冲突输：故意让你输
        if (isBetBig) {
      ...
      } else if (finalOutcome === 'lucky' || finalOutcome === 'conflict_win') {
        // 运气爆棚/冲突赢：强制让你赢
        if (isBetBig) {
          resultNum = Math.floor(Math.random() * 3) + 4; // 4-6 (Big)
        } else {
      ...
      roundData.outcomeInfo = {
          type: finalOutcome,
          cheatProb: this.cheatProbability,
          luckProb: this.luckProbability
      };
      // 直接修改对象引用
  ...
  consume () {
    ...
    // --- 在消费时判断输赢 ---
    const { id, betBig, result, outcomeInfo } = data;
    const betStr = betBig ? '大' : '小';

    // 打印开奖原因
    if (outcomeInfo) {
        const { type, cheatProb, luckProb } = outcomeInfo;
        if (type === 'conflict_win') {
            console.log(`!!! 巅峰对决：运气(${luckProb.toFixed(2)}) 战胜 千术(${cheatProb.toFixed(2)}) -> 强制玩家赢 !!!`);
        } else if (type === 'conflict_loss') {
            console.log(`!!! 巅峰对决：千术(${cheatProb.toFixed(2)}) 碾压 运气(${luckProb.toFixed(2)}) -> 强制玩家输 !!!`);
        } else if (type === 'cheat') {
             console.log(`!!! 庄家出千！(你压${betBig ? '大' : '小'}，强制开${betBig ? '小' : '大'}) !!!`);
        } else if (type === 'lucky') {
             console.log(`!!! 鸿运当头！(强制玩家赢) !!!`);
        }
    }
...
console.log('游戏开始！消费完一单才会下下一单...');
const game = new GamblingQueue();
