...
- Hook点：在特定时机执行（如 beforeCompile、afterBuild）
- 命令点：注册可执行的命令（如 CLI 命令、菜单项）
- UI扩展点：注入UI组件（如工具栏按钮、侧边栏面板）
- 数据转换点：处理数据流（如 Babel 的 AST 转换）

上下文:
上下文(Context)是插件执行时的环境信息，包括：
- 系统API：核心系统提供的接口（如文件系统、网络请求）
- 配置信息：当前运行配置、环境变量
- 共享状态：全局状态、事件总线
- 依赖注入：其他插件的服务、工具函数
...
- 沙箱隔离：限制插件可访问的API和资源（如 iframe、Web Worker、Proxy）
   ...
   function detectVersionConflict(pluginA, pluginB) {
     ...
     for (const [name, versionA] of Object.entries(depsA)) {
       if (depsB[name] && !satisfies(versionA, depsB[name])) {
         return { conflict: true, plugin: name, versions: [versionA, depsB[name]] };
       }
     }
...
1. 白名单机制：
   ...
   const API_WHITELIST = {
     ...
     'storage.write': (context) => context.permissions.includes('storage.write'),
...
API 设计示例:
  ...
  class PluginHookSystem {
    ...
    registerPlugin(plugin) {
      ...
      // 异步 hook
      this.hooks.transform.tap(plugin.name, async (code, filename) => {
   ...
   class PerformanceBudget {
     ...
     async check(fn) {
       ...
       try {
         ...
         if ((endMemory - startMemory) / 1024 / 1024 > this.budget.memoryLimit) {
...
2. 错误监控：
   ...
   class ErrorMonitor {
     ...
     reportError(error) {
       ...
       // 本地统计
       this.errorStats[error.plugin] = 
         (this.errorStats[error.plugin] || 0) + 1;
     }
   }
   ```
