...
核心机制：
  - 路由劫持：监听 URL 变化，加载/卸载应用
  - 生命周期：子应用需导出 bootstrap、mount、unmount
  - 应用注册：registerApplication() 注册子应用
...
优点：
  ...
  ✅ 自动 JS 沙箱（Proxy/快照沙箱）
  ✅ 自动 CSS 隔离（Shadow DOM/scoped CSS）
  ✅ 预加载能力（prefetch）
...
核心机制：
  1. JS 沙箱：
     - ProxySandbox（单例，推荐）
     - SnapshotSandbox（快照，IE 兼容）
     - LegacySandbox（遗留模式）
  
  2. CSS 隔离：
     - Shadow DOM（严格隔离）
     - Scoped CSS（动态添加作用域）
     - experimentalStyleIsolation（实验性）
  
  3. 资源加载：
     - HTML Entry（加载子应用 HTML）
     - 提取 JS/CSS 并插入到主应用
...
适用场景：
  - 中后台系统（qiankun 的主战场）
  - 需要开箱即用的沙箱隔离
  - 团队技术栈多样化（React + Vue 混用）
  - 蚂蚁系技术栈（Umi + Ant Design）
...
|---------------|-------------|-------------|-------------|-------------------|
| 隔离能力       | 天然隔离     | 无           | Proxy 沙箱   | 无                |
| 性能          | 差           | 好           | 中等         | 好                |
| 学习成本       | 低           | 高           | 中等         | 中等              |
| 改造成本       | 低           | 高           | 中等         | 高                |
| 技术栈要求     | 无           | 无           | 无           | Webpack 5         |
| 依赖共享       | 不支持       | 手动         | 手动         | 自动（shared）    |
| CSS 隔离      | 天然隔离     | 需手动       | 自动         | 需手动            |
| 通信机制       | postMessage | 自定义       | initGlobalState | 直接导入       |
| 适用粒度       | 页面级       | 应用级       | 应用级       | 组件级/应用级     |
| 首屏性能       | 差           | 好           | 中等         | 好                |

选型建议：
1. 嵌入第三方页面 → iframe
2. 中后台多团队协作 → qiankun（首选）
3. 需要高度定制 → single-spa
4. 组件级复用、技术栈统一 → Module Federation
5. 性能要求高、技术栈统一 → Module Federation
6. 团队技术能力弱 → qiankun（开箱即用）

组合方案（最佳实践）：
- qiankun（应用级隔离） + Module Federation（组件级复用）
- 核心业务用 qiankun，组件库用 Module Federation
```
...
问题：
- 不同子应用的全局变量冲突（window.xxx）
- 子应用污染主应用的全局对象
- 第三方库冲突（如都引入了 jQuery，但版本不同）

解决方案：

1. Proxy 沙箱（推荐，qiankun 默认）：
   - 原理：为每个子应用创建一个 Proxy 代理 window 对象
   - 子应用的全局变量读写都被拦截
   - 不影响真实的 window 对象
...
3. with + new Function（不推荐）：
   - 使用 with 改变作用域
   - 性能差，严格模式不支持

4. iframe 隔离（天然隔离，但性能差）
...
1. Shadow DOM（严格隔离）：
   - 原理：创建独立的 DOM 树，CSS 完全隔离
   - 优点：隔离彻底
   - 缺点：弹窗等脱离 Shadow DOM 的元素无法继承样式
...
2. Scoped CSS（动态添加属性）：
   - 原理：为每个子应用的样式添加唯一前缀
   - qiankun 实现：动态添加 data-qiankun="app1" 属性
   - 样式规则改写：.btn → div[data-qiankun="app1"] .btn
...
5. 命名空间（约定）：
   - 约定子应用添加前缀：.app1-button、.app2-button
   - 依赖人工规范，容易遗漏

最佳实践：
- 主应用：使用 CSS Modules 或 CSS-in-JS
- 子应用：Scoped CSS（qiankun 自动处理）
- 第三方组件库：通过配置加前缀
...
问题：
- 子应用 DOM 操作冲突
- document.querySelector 可能选中其他应用的元素
- 全局事件监听冲突
...
2. 路由劫持：
   ...
   window.addEventListener('popstate', () => {
     const activeApp = matchApp(location.pathname)
     loadApp(activeApp)
   })
...
问题：
- 不同子应用的状态管理冲突
- Redux/MobX 全局 store 冲突
- LocalStorage/SessionStorage 冲突
...
2. 发布订阅模式：
   ...
   // 主应用
   window.__EVENT_BUS__ = new EventEmitter()
   
   // 子应用
   window.__EVENT_BUS__.on('login', (user) => {})
   window.__EVENT_BUS__.emit('login', user)

3. LocalStorage 隔离：
   - 添加应用前缀
   - app1_token、app2_token
...
1. 统一请求封装（主应用提供）：
   ...
   // 主应用
   window.__REQUEST__ = axios.create({
     interceptors: { /* 统一拦截器 */ }
   })
   
   // 子应用
   window.__REQUEST__.get('/api/users')
...
5. 域名隔离（微服务）：
   - 不同子应用请求不同域名
   - app1 → api1.example.com
   - app2 → api2.example.com

最佳实践：
- 公共请求（登录、用户信息）：主应用统一管理
- 业务请求：子应用独立管理
- 拦截器：子应用独立配置，避免冲突
```
...
```
Proxy 沙箱（ProxySandbox，推荐）：

原理：
- 为每个子应用创建一个 Proxy 代理的 fakeWindow
- 拦截所有对 window 的读写操作
- 读取时：优先从 fakeWindow 读，没有则从真实 window 读
- 写入时：只写入 fakeWindow，不污染真实 window
- 支持多实例（多个子应用同时运行）
...
class ProxySandbox {
  constructor(name) {
    ...
    const proxy = new Proxy(fakeWindow, {
      get(target, key) {
        // 优先从代理对象取值，否则从 window 取
        if (key in target) {
          return target[key]
        }
        const value = window[key]
        // 绑定 this（如 window.addEventListener 需要绑定到真实 window）
...
✅ 支持多实例（多个子应用同时运行）
✅ 性能好（Proxy 拦截，无需遍历）
✅ 卸载干净（fakeWindow 可被 GC 回收）

缺点：
❌ 不支持 IE（Proxy 无法 polyfill）
❌ 某些场景下拦截不到（如 document.createElement('script')）
❌ 需要处理 this 绑定问题
...
快照沙箱（SnapshotSandbox，IE 兼容）：

原理：
- 激活时：遍历 window，保存快照（snapshot）
- 运行时：子应用直接修改 window（无隔离）
- 卸载时：对比 window 和快照，恢复被修改的属性
- 单实例模式（同一时间只能运行一个子应用）
...
缺点：
❌ 性能差（需要遍历整个 window 对象，属性可能有上千个）
❌ 污染全局（运行时直接修改 window）
❌ 单实例（同一时间只能运行一个子应用）
❌ 无法记录属性删除（delete window.xxx）
...
Legacy 沙箱（LegacySandbox，已废弃）：

原理：
- 类似 Proxy 沙箱，但使用 Proxy 拦截 window
- 单实例模式
- 记录 window 的新增/修改/删除属性
- 卸载时恢复 window
...
class LegacySandbox {
  constructor() {
    ...
    this.modifiedPropsMap = {}  // 修改属性（记录原始值）
    this.currentPropsMap = {}   // 当前属性值
    ...
    const proxy = new Proxy(fakeWindow, {
      ...
      set(target, key, value) {
        if (!window.hasOwnProperty(key)) {
          // 新增属性
          addedPropsMap[key] = value
        } else if (!modifiedPropsMap.hasOwnProperty(key)) {
          // 首次修改，记录原始值
          modifiedPropsMap[key] = window[key]
        }
        
        currentPropsMap[key] = value
        window[key] = value
        return true
      }
    })
  ...
  active() {
    ...
    Object.keys(this.currentPropsMap).forEach(key => {
      window[key] = this.currentPropsMap[key]
    })
  }
  
  inactive() {
    // 删除新增的属性
    Object.keys(this.addedPropsMap).forEach(key => {
      delete window[key]
    })
...
1. 浏览器支持判断：
   if (window.Proxy) {
     ...
     // 降级使用 SnapshotSandbox
   }

2. qiankun 自动选择逻辑：
   - 默认：ProxySandbox（现代浏览器）
   - 降级：SnapshotSandbox（不支持 Proxy）
   - 配置：可手动指定
   
   start({
     sandbox: {
       loose: true  // 使用 SnapshotSandbox
     }
   })
...
3. 实际项目选择：
   ...
   场景2：需要兼容 IE
   → SnapshotSandbox（loose: true）
   
   场景3：单应用模式
   → 两者都可以，优先 ProxySandbox
   
   场景4：对性能要求极高
   → 考虑不使用沙箱，约定子应用规范开发

4. 性能对比：
   ProxySandbox：★★★★★（Proxy 拦截，无遍历）
   LegacySandbox：★★★★☆（记录变更）
   SnapshotSandbox：★★☆☆☆（遍历 window，慢）

5. 隔离程度对比：
   ProxySandbox：★★★★★（完美隔离）
   LegacySandbox：★★★☆☆（运行时污染）
   SnapshotSandbox：★★☆☆☆（运行时污染）

6. 最佳实践：
   - 生产环境：ProxySandbox（放弃 IE 支持）
   - 必须支持 IE：SnapshotSandbox + 单应用模式
   - 开发环境：ProxySandbox（便于调试）

7. 沙箱局限性（所有方案共同的）：
   - 无法拦截 document.createElement('script')
   - 无法拦截 eval()、new Function()
   - 无法拦截 addEventListener（需要额外处理）
   - 无法拦截 setTimeout/setInterval（需要劫持）
   
   解决方案：
   - 约定子应用规范开发
   - 使用沙箱 + 约定的组合方案
   - 通过 import-html-entry 解析脚本，而非直接执行
```
...
2. 运行时加载流程：
   ...
   步骤1：主应用启动，加载远程应用的 remoteEntry.js
   <script src="http://localhost:3001/remoteEntry.js"></script>
   
   步骤2：remoteEntry.js 注册到全局
   window.app1 = {
     get: (module) => { /* 加载函数 */ },
     init: (shared) => { /* 初始化共享依赖 */ }
   }
   ...
   步骤4：Webpack 运行时拦截 import，调用远程加载函数
   window.app1.get('./Button').then(module => {
     // 返回模块
   })
   
   步骤5：远程应用异步加载模块代码（chunk）
   - 加载 Button.js
   - 解析依赖（如 react、lodash）
   - 返回模块
...
3. 核心机制：
   ...
   b) Remote（远程加载）：
      ...
      // 使用
      import Button from 'app1/Button'  // 自动异步加载
...
3. 共享策略配置项：
   ...
   b) eager（立即加载）：
      ...
      - true：同步加载（打包到主 chunk）
      - false：异步加载（懒加载）
      
      适用：核心依赖（如 React）
   
   c) requiredVersion（版本要求）：
      requiredVersion: '^18.0.0'
      
      - 指定兼容的版本范围
      - 不满足会告警或加载多份
   
   d) strictVersion（严格版本）：
      strictVersion: true
...
1. 冲突场景：
   ...
   场景1：主应用 React 18.2.0，子应用 React 17.0.0
   - 不兼容（major 版本不同）
   - 需要加载两份
   
   场景2：主应用 React 18.2.0，子应用 React 18.1.0
   - 兼容（minor 版本兼容）
   - 只加载一份（使用 18.2.0）
   
   场景3：主应用 Ant Design 4.x，子应用 Ant Design 5.x
   - 不兼容
   - 需要加载两份
...
2. Webpack 版本协商算法：
   ...
   步骤4：singleton 冲突处理
   - 如果设置了 singleton: true
   - 版本不兼容会告警，但强制使用一个版本
   - strictVersion: true 则直接报错
...
3. 冲突处理策略：
   ...
   requiredVersion: '^18.0.0'  // 18.x.x 都兼容
   ...
   singleton: true
   strictVersion: false  // 宽松模式

4. 实际案例：
   
   案例1：React 18 和 React 17 共存
   问题：子应用使用 React 17，主应用使用 React 18
   ...
   缺点：体积增加，可能有两个 React 实例
   
   案例2：Ant Design 版本冲突
   问题：不同子应用使用不同版本的 Ant Design
...
5. 监控与告警：
   ...
   window.addEventListener('error', (e) => {
     if (e.message.includes('Shared module')) {
       // 上报版本冲突错误
       reportError('版本冲突', e.message)
     }
   })

6. 最佳实践总结：
   
   ✅ 优先统一版本（通过 Monorepo 或约定）
   ✅ 核心库设置 singleton（React、Vue）
   ✅ 使用宽松的版本范围（^18.0.0）
   ✅ 工具库可以不共享（如 lodash）
   ✅ 定期升级依赖，保持一致
   ✅ 建立版本管理规范
   
   ❌ 避免强制 singleton + strictVersion（容易报错）
   ❌ 避免过度共享（并非所有依赖都要共享）
   ❌ 避免版本范围过宽（可能不兼容）
```
