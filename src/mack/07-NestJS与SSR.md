...
- 方式1：在路由组件中定义静态方法（如 getServerSideProps、load）
- 方式2：在服务端路由处理函数中直接调用数据获取逻辑
- 方式3：使用数据获取库（如 React Query、SWR）的服务端适配
- 数据获取完成后，将数据作为 props 传递给组件
- 数据通常序列化到 HTML 中（如 window.__INITIAL_STATE__），供客户端 hydration 使用
